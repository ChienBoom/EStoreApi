...
order.get('/orderproducts', checkToken, (req, res) =>{
  try{
    let orderId = req.body.orderId;
    pool.query(`select orderdetails.*, products.product_name from orderDetails, products where orderDetails.productsId = products.id and orderId=${orderId}`, (error, orderProducts) => {
      if(error){
        res.status(500).send({
          error: error.code,
          message: error.message
